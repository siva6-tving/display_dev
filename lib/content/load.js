...
$('#top-gnb').ready(()=>{
	_.each(findLocationInfo(LOC, location), v=>{
		...
		$('.warnning').css('margin-left', $('header > aside').length?'20pt':'auto').css('background-color', v.color).on('click', function() {
			...
			// $.post('https://www.koreabaseball.com/ws/Schedule.asmx/GetTodayGame', {gameDate: '20240724', leId: 1, srId:'0,1,3,4,5,7', headerCk:0}, data => {
			// 	console.log(data);
			// });
			...
			const playTeamSong = () => {
				...
				if(song && $audio.length === 0) {
					...
					$this.find('audio').get(0).volume = 0.2;
			...
			const getPlayerCodeFromDiv = $div => {
				const playerImg = $div.find('> img[src^="https://image.tving.com/ntgs/sports/kbo/player/"]');
			...
			$('body').on('click', 'div', function() {
				const playerCode = getPlayerCodeFromDiv($(this));
